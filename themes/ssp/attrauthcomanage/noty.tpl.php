...
<div class="noty-description">
    ...
    foreach ($this->data['noty']['description'] as $dictKey => $placeholders) {
        if (!empty($placeholders)) {
            echo $this->t('{themeegi:attrauthcomanage_noty:' . $dictKey . '}', $placeholders);
        } else {
            echo $this->t('{themeegi:attrauthcomanage_noty:' . $dictKey . '}');
        }
    }
    ?>
</div>
...
<script type="application/javascript">
    ...
    function form_ajax(frm_ob) {
        ...
        jqxhr.done((data, textStatus, xhr) => {
            ...
            $('.invite-expire')
            .html('<?= htmlspecialchars($this->t('{themeegi:attrauthcomanage_noty:invitation_restored}')) ?>');
